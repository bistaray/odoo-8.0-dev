        ...
        query="""
               SELECT cast(100000000000 + av.id as bigint) as id,rp.id as partner_id, rp.name as partner_name, days_due as avg_days_overdue, 
                      ...
                      CASE WHEN (select count(name) from account_move_line where name=av.number) >= 1 
                           ...
                           ELSE (aml.debit-aml.credit) END as total,          
                      CASE WHEN (days_due BETWEEN 31 AND 60) THEN 
                           CASE WHEN (select count(name) from account_move_line where name=av.number) >= 1
                           THEN (select -1 * (sum(credit) - sum(debit)) from account_move_line where name =av.number)
                           ELSE (aml.debit-aml.credit) END END  AS "days_due_31to60",
                      CASE WHEN (days_due BETWEEN 61 AND 90) THEN 
                           CASE WHEN (select count(name) from account_move_line where name=av.number) >= 1
                           THEN (select -1 * (sum(credit) - sum(debit)) from account_move_line where name =av.number)
                           ELSE (aml.debit-aml.credit) END END  AS "days_due_61to90",
                      CASE WHEN (days_due BETWEEN 91 AND 120) THEN 
                           CASE WHEN (select count(name) from account_move_line where name=av.number) >= 1
                           THEN (select -1 * (sum(credit) - sum(debit)) from account_move_line where name =av.number)
                           ELSE (aml.debit-aml.credit) END END AS "days_due_91to120",
                      CASE WHEN (days_due >= 121) THEN 
                           CASE WHEN (select count(name) from account_move_line where name=av.number) >= 1
                           THEN (select -1 * (sum(credit) - sum(debit)) from account_move_line where name =av.number)
                           ELSE (aml.debit-aml.credit) END END  AS "days_due_121togr",
                      0 AS "unapp_credits",
                      (aml.debit-aml.credit) AS "unapp_cash",
                      CASE when days_due < 0 THEN 0 ELSE days_due END as "max_days_overdue",
                      CASE WHEN (days_due < 1) THEN 
                           CASE WHEN(select count(name) from account_move_line where name=av.number) >= 1 
                           THEN (select -1 * (sum(credit) - sum(debit)) from account_move_line where name =av.number)
                           ELSE (aml.debit-aml.credit) END END  AS "current",
                      CASE WHEN (days_due BETWEEN 1 AND  30) THEN 
                           CASE WHEN (select count(name) from account_move_line where name=av.number) >= 1
                           THEN (select -1 * (sum(credit) - sum(debit)) from account_move_line where name =av.number)
                           ELSE (aml.debit-aml.credit) END END  AS "days_due_01to30",    
               ...
               INNER JOIN
                  ( SELECT id, current_date - aml2.date_maturity AS days_due  FROM account_move_line aml2 ) DaysDue
