...
class partner_aging_supplier(osv.osv):
    ...
    def init(self, cr):
        ...
        query = """
                select aml.id, partner_id, NULL AS partner_name, days_due AS avg_days_overdue, NULL as date, date_maturity as date_due,                 
                ...
                select id, partner_id, partner_name, avg_days_overdue, NULL as date, date_due, total, unapp_cash,
                       days_due_31to60, days_due_61to90, days_due_91to120, days_due_121togr, current, days_due_01to30, max_days_overdue, invoice_ref, invoice_id, comment, unapp_credits
                       from (            
                SELECT l.id as id, l.partner_id as partner_id, res_partner.name as "partner_name",
                    ...
                    CASE WHEN (days_due <= 0) and ai.id is not null THEN 
                         CASE WHEN ai.type = 'in_refund' THEN -1*ai.residual*ABS((l.debit - l.credit)/ai.amount_untaxed) ELSE ai.residual*ABS((l.debit - l.credit)/ai.amount_untaxed) END 
                         WHEN (days_due <= 0) and ai.id is null THEN l.credit - l.debit 
                         ELSE 0 END  AS "current",                                
